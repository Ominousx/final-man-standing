...
export default function Home() {
  ...
  const generateRandomMatch = () => {
    ...
    // Filter out matches that user has already picked
    const userPicks = picks.filter(p => p.userEmail === user?.email);
    const pickedMatchIds = userPicks.map(p => p.match_id);
    const availableMatches = matches.filter(m => !pickedMatchIds.includes(m.match_id));
    
    if (availableMatches.length === 0) {
      // All matches have been picked, show a random one
      const randomIndex = Math.floor(Math.random() * matches.length);
      setCurrentRandomMatch(matches[randomIndex]);
    ...
    setCurrentRandomMatch(availableMatches[randomIndex]);
    setUserPick("");
      ...
      <div className="w-64 bg-black border-r border-gray-800 flex flex-col">
        ...
        <div className="p-6 border-b border-gray-800">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-white">VCT Survivor</h1>
